...
const Ajuda: React.FC = () => {
  ...
  const [ajudaTheme, setAjudaTheme] = useState<'dark' | 'light'>('dark');
  ...
  useEffect(() => {
    fetch('/AJUDA.md')
      ...
      .catch(() => {
        setMarkdown(exemploAjuda);
        setHeadings(extractHeadings(exemploAjuda));
        setLoading(false);
      });
  }, []);
  ...
  const muiTheme = createTheme({
    palette: {
      mode: ajudaTheme,
      background: { default: ajudaTheme === 'dark' ? '#181820' : '#fff' },
      text: { primary: ajudaTheme === 'dark' ? '#fff' : '#181820' },
    },
  });
  ...
  return (
    <ThemeProvider theme={muiTheme}>
      ...
      <Box p={3} sx={{ maxWidth: 1200, margin: '0 auto', display: 'flex', flexDirection: 'column', background: ajudaTheme === 'dark' ? '#181820' : '#fff', color: ajudaTheme === 'dark' ? '#fff' : '#181820' }}>
        {/* Barra de ações */}
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 1, gap: 2 }}>
          <Typography variant="h6" sx={{ color: '#FFA500', fontWeight: 600, mr: 2 }}>
            Índice
          </Typography>
          <IconButton onClick={handlePrint} aria-label="Imprimir ajuda" size="small">
            <PrintIcon />
          </IconButton>
          <IconButton onClick={() => setAjudaTheme(ajudaTheme === 'dark' ? 'light' : 'dark')} aria-label="Alternar modo claro/escuro" size="small">
            {ajudaTheme === 'dark' ? <LightModeIcon /> : <DarkModeIcon />}
          </IconButton>
        </Box>
        {/* Breadcrumb */}
        ...
        <FormControl fullWidth size="small" sx={{ mb: 2, maxWidth: 420 }}>
          <Autocomplete
            ...
            renderInput={(params) => (
              <TextField {...params} label="Selecione ou busque um tópico..." size="small" sx={{ background: ajudaTheme === 'dark' ? '#181820' : '#fff', borderRadius: 2 }} />
            )}
